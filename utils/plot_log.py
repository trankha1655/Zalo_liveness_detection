...
def draw_log(args, epoch, epoch_list, lossTr_list, Acc_list, lossVal_list):
    ...
    if args.val_epochs == 1:
        try:
            ...
            ax2.plot(range(1, epoch + 1), Acc_list, label="Val Acc")
            ax2.set_title("Average Acc vs epochs")
            ax2.set_xlabel("Epochs")
            ax2.set_ylabel("Current Acc")
            ax2.legend()
            plt.savefig(args.savedir + "mAcc.png")
            plt.close('all')
    else:
        # plt loss
        fig1, ax1 = plt.subplots(figsize=(11, 8))
        try:
            ...
            # plt Acc
            fig2, ax2 = plt.subplots(figsize=(11, 8))
            ax2.plot(epoch_list, Acc_list, label="Val Acc")
            ax2.set_title("Average Acc vs epochs")
            ax2.set_xlabel("Epochs")
            ax2.set_ylabel("Current Acc")
            ax2.legend()
            plt.savefig(args.savedir + "Acc.png")
            plt.close('all')